...
const hookKinesis = require('./kinesis');
        ...
        request(requestParams, maybeCallback)
          .on('request', function(req) {
            ...
            req.on('response', function updateLatency(res) {
              ...
              const currentDate = ((new Date()).toISOString().replace('T', ' ')).split(' ');

              hookKinesis({
                Data : JSON.stringify({
                  phase: 'test2-pack-50K-37',
                  url: requestParams.url,
                  statusCode: code,
                  responseTime: (delta / 1000000),
                  datetz: (currentDate.join(' ')).replace('Z', ''),
                  date: currentDate[0],
